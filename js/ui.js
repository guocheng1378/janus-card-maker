...
function generateTplThumbnail(tpl) {
  ...
  try {
    ...
    var result = '<div style="width:' + w + 'px;height:' + h + 'px;border-radius:6px;overflow:hidden;position:relative;transform:scale(1);flex-shrink:0">' +
...
function setupEvents() {
  ...
  document.getElementById('cfgContent').addEventListener('change', function (e) {
    var t = e.target;
    var changed = false;
    if (t.dataset.cfg && t.tagName === 'SELECT') { _cfg[t.dataset.cfg] = t.value; _dirty = true; changed = true; }
  ...
  document.addEventListener('keydown', function (e) {
    ...
    if (e.ctrlKey && e.key === 'z' && !e.shiftKey && !isInCodeEditor) { e.preventDefault(); undo(); }
    if (e.ctrlKey && (e.key === 'y' || (e.key === 'z' && e.shiftKey)) && !isInCodeEditor) { e.preventDefault(); redo(); }
  ...
  // ── 颜色 hex 输入框同步 ──
  document.getElementById('cfgContent').addEventListener('input', function (e) {
    var hexInput = e.target.closest('.color-hex-input');
    if (!hexInput) return;
    var val = hexInput.value.trim();
    if (/^#[0-9a-fA-F]{6}$/.test(val)) {
      var prop = hexInput.dataset.colorSync;
      var cidx = Number(hexInput.dataset.cidx);
      if (cidx >= 0 && cidx < _elements.length) {
        _elements[cidx][prop] = val;
        // 同步 color picker
        var picker = hexInput.parentElement.querySelector('input[type="color"]');
        if (picker) picker.value = val;
        var cv = hexInput.parentElement.querySelector('.color-val');
...
JCM._doFilterTemplates = function (query) {
  ...
  cards.forEach(function (card) {
    ...
    var searchMatch = !query || name.indexOf(query) >= 0 || desc.indexOf(query) >= 0 || tplId.indexOf(query) >= 0;
